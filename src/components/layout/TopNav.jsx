...
const TopNav = ({ show = true }) => {
  ...
  return (
    <>
      <nav className={`fixed top-0 w-full p-6 md:p-10 z-[200] flex justify-between items-center transition-all duration-1000 ${show ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-10 pointer-events-none'
        }`}>
        ...
        {/* MOBILE MENU OVERLAY */}
        <div className={`fixed inset-0 bg-black flex flex-col items-center pt-40 transition-all duration-500 md:hidden z-[190] ${isMenuOpen ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0 pointer-events-none'
          }`}>
          {/* CRT & GLOW EFFECTS */}
            ...
            {navLinks.map((item, index) => {
              ...
              return (
                <a
                  ...
                  className={`text-5xl font-black uppercase tracking-widest transition-all duration-500 transform ${isMenuOpen ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
